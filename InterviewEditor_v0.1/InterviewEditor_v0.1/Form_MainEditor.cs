...
namespace InterviewEditor_v0._1
{
    public partial class Form_MainEditor : Form
    {
        ...
        // ----------------------------------------------------------------------------------------------------
        // Function: Finds all beats that are still "__NEW__".
        // ----------------------------------------------------------------------------------------------------
        void findUnwrittenBeats()
        {
            // Initialize the unwritten list.
            List<StoryBeat> unwrittenBeats = new List<StoryBeat>();

            // Go through all of the beats in the list.
            for (int index = 0; index < storyBeatList.Count; ++index)
            {
                // Check if the main text is "__NEW__".
                if (storyBeatList[index].main_text == "__NEW__")
                {
                    // Add the current beat to the list.
                    unwrittenBeats.Add(storyBeatList[index]);
                }
                ...
                {
                    // Check if the next beat for choice 1 is the selected beat.
                    if (((StoryBeatChoice)storyBeatList[index]).choices[0].choice_text == "__NEW__")
                    {
                        unwrittenBeats.Add(storyBeatList[index]);
                    }

                    // Otherwise, check if the next beat for choice 2 is the selected beat.
                    else if (((StoryBeatChoice)storyBeatList[index]).choices[1].choice_text == "__NEW__")
                    {
                        unwrittenBeats.Add(storyBeatList[index]);
                    }

                    // Otherwise, check if the next beat for choice 3 is the selected beat.
                    else if (((StoryBeatChoice)storyBeatList[index]).choices[2].choice_text == "__NEW__")
                    {
                        unwrittenBeats.Add(storyBeatList[index]);
                    }
                }
            ...
            // Check if any unwritten beats were actually found.
            if (unwrittenBeats.Count > 0)
            {
                loadSpecifiedBeatTable(unwrittenBeats, "Interview Editor - Unwritten Beats");
            }

            // Otherwise, no unwritten beats were found, so just show a message on the status strip.
            else
            {
                label_statusStrip.Text = "Unable to find any unwritten beats.";
            }
        }
        // ----------------------------------------------------------------------------------------------------
        ...
        // Event: Menu: View > Find unwritten beats.
        private void findUnwrittenBeatsToolStripMenuItem_Click(object sender, EventArgs e)
        {
            findUnwrittenBeats();
        }
    }
